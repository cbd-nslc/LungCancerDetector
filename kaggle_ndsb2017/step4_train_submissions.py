    ...
    for index, row in labels_df.iterrows():
        ...
        for magnification in [1, 1.5, 2]:
            ...
            for extension in extensions:
                src_dir = settings.NDSB3_NODULE_DETECTION_DIR + "predictions" + str(
                    int(magnification * 10)) + extension + "/"
            ...
            row_items += [nodule_max, nodule_chance, nodule_count, nodule_median, nodule_wmax, coord_z, second_largest,
                          coord_y, coord_x]
...
def train_xgboost_on_combined_nodules_ensembletest(fixed_holdout=False, submission_is_fixed_holdout=False,
                                                   ensemble_lists=[]):
        ...
        for variant in variants:
            ...
            if True:
                clf = xgboost.XGBRegressor(max_depth=4,
                                           ...
                                           seed=seed)
                #
                clf.fit(x_train, y_train, verbose=fixed_holdout and False,
                        eval_set=[(x_train, y_train), (x_holdout, y_holdout)], eval_metric="logloss",
            ...
            print(score, "\tbest:\t", clf.best_score, "\titer\t", clf.best_iteration, "\tmean:\t", train_preds_list[-1],
                  "\thomean:\t", y_holdout.mean(), " variant:", variant)
...
def train_xgboost_on_combined_nodules(extension, fixed_holdout=False, submission=False,
    ...
    if submission_is_fixed_holdout:
        print("First 300 patients score: ", sum(scores) / len(scores), " mean chance: ",
              sum(train_preds_list) / len(train_preds_list))
    else:
        print("Average score: ", sum(scores) / len(scores), " mean chance: ",
              sum(train_preds_list) / len(train_preds_list))
...
if __name__ == "__main__":
    if True:
        for model_variant in ["_luna16_fs", "_luna_posnegndsb_v1", "_luna_posnegndsb_v2"]:
            ...
            if True:
                train_xgboost_on_combined_nodules(fixed_holdout=False, submission=True,
                                                  submission_is_fixed_holdout=False, extension=model_variant)
