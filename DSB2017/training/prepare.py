...
import shutil
import numpy as np
from DSB2017.config import config_training
...
from DSB2017.preprocessing.step1 import step1_python
import warnings
...
def full_prep(step1=True, step2=True):
    ...
    prep_folder = config_training['preprocess_result_path']
    data_path = config_training['stage1_data_path']
    finished_flag = '.flag_prepkaggle'

    if not os.path.exists(finished_flag):
        alllabelfiles = config_training['stage1_annos_path']
        tmp = []
        ...
        filelist = os.listdir(config_training['stage1_data_path'])
...
def preprocess_luna():
    luna_segment = config_training['luna_segment']
    savepath = config_training['preprocess_result_path']
    luna_data = config_training['luna_data']
    luna_label = config_training['luna_label']
...
def prepare_luna():
    ...
    luna_raw = config_training['luna_raw']
    luna_abbr = config_training['luna_abbr']
    luna_data = config_training['luna_data']
    luna_segment = config_training['luna_segment']
    ...
    if not os.path.exists(finished_flag):
        ...
        abbrevs = np.array(pandas.read_csv(config_training['luna_abbr'], header=None))
